...
def _ssim(
    ...
    sigma1_sq = compensation * (gaussian_filter(X * X, win) - mu1_sq) + EPSILON
    sigma2_sq = compensation * (gaussian_filter(Y * Y, win) - mu2_sq) + EPSILON
    sigma12 = compensation * (gaussian_filter(X * Y, win) - mu1_mu2) + EPSILON
    # print(sigma1_sq, sigma2_sq, sigma12)
    # print(mu1_sq, mu2_sq, mu1_mu2)
    ...
    if full:
        ssim_val_clamped = torch.clamp(ssim_val, min=1e-6)
        return ssim_val_clamped, cs
    else:
        return ssim_val
...
def ms_ssim(
    ...
    msssim_val = torch.prod(
        (mcs[:-1] ** weights[:-1].unsqueeze(1)) * (ssim_val ** weights[-1]),
        dim=0,
    )
